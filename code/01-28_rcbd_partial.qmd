...
```{r setup}
# Loading packages
library(tidyverse) # for data wrangling and plotting. it includes ggplot2, tidyr etc.
library(car) # for Anova function
library(lme4) # to run our model. Most popular to run mixed model and anova models
...
```{r glimpse}
glimpse(rcbd_df) #dbl means numerical and chr means character. This can impact your analysis if misrepresented. if it numerical for instance in this case block is numeric. it does not have any meaning in that sense cuz R will calcualte intercept and slop of all numerics. We need to convert it to character. you can run both anova and regression analysis for the same data but it comes down to your goal. For regression, model is calculating intercept and slop while for anova model gives you mean. Regression analysis helps us understand what would be the effect of the dosages we did not apply.
```
# c) Wrangling
```{r rcbd_dfw}
rcbd_dfw <- rcbd_df %>%
...
# if we had not changed n-rate from numerical to factor we would have gotten one big boxplot.jitter lets you see the point which overlap and it moves them left or right.
```
...
#difference between crd and rcbd model is blocks which is added here in the model as rep. The restriction forced the rep 4 to stop and it is the sum of 1,2,3. It is because of the restriction sum to zero. The last one is sum to zero.
# Summary
summary(rcbd_mod)
```
...
#if all are significant then go for only interaction part. IF main effects are significant and interaction is not then you go for only main effects.
```
...
variance error ~ eiidN (0, sigma2), this sentence breaks down the assumptions of linear model.
...
#if we use colon then it is all against all and not separately in case of '|'.
rcbd_means_all
```
...
```{r rcbd final plot}
#we wont sepcify data at ggplot level cuz we are gonna use two data sets, one raw data and one pairwise for letters
ggplot() + 
  ...
  theme(legend.position = "none")
#if we wanna change only one thing in the theme then we need to use simple theme other theme_ and choose  
#geomlable adds a background in the back of letters while geom_text does not. 
#There is also a scale_color_manual to manually choose the color. You can either choose color separately or give hex.
