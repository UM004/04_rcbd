...
# Model fitting
rcbd_mix_mod <- lmer(yield_kgha ~ nrate_kgha*krate_kgha + (1|rep),
                     data = rcbd_dfw)
                     
#if you wanna analyze for multiple locations then you got to have them in same model, nest rep in location(location/rep). The larger hierarchy factor comes first. Same applies for year. For year, location and rep, (1|year:location/rep) or create a new column with combined year and location but that would give least error.
# Summary
...
#there are no reps being shown here. ANOVA only shows us fixed effect stuff. Rep is being treated as random effect here and ANOVA only shows fixed efffects thats why.
```
